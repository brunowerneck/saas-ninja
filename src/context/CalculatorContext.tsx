...
interface CalculatorContextType {
  ...
  addSubscriptionPlan: (name: string, price: number, currency: Currency) => void;
...
const initialState: CalculatorState = {
  dollarRate: 5.0,
  subscriptionPlans: [
    { id: uuidv4(), name: 'Basic Plan', price: 29.90, currency: 'BRL' },
  ],
  monthlyCosts: [
    { id: uuidv4(), name: 'Hosting', value: 20, currency: 'USD' },
  ],
  annualCosts: [
    { id: uuidv4(), name: 'Domain', value: 15, currency: 'USD' },
  ],
  perUserCosts: [
    { id: uuidv4(), name: 'Infraestrutura', value: 2, currency: 'BRL' },
  ],
  paymentGatewayPercentage: 2.5,
  paymentGatewayFixed: 0.50,
  taxRate: 15,
...
export const CalculatorProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const calculateNetRevenuePerUser = (grossRevenue: number): number => {
    ...
    const afterGatewayFixed = afterGatewayPercentage - state.paymentGatewayFixed;
  ...
  const addSubscriptionPlan = (name: string, price: number, currency: Currency) => {
    setState(prev => ({
      ...prev,
      subscriptionPlans: [...prev.subscriptionPlans, { id: uuidv4(), name, price, currency }]
    }));
  };
  ...
  const setActiveTab = (tab: 'subscriptions' | 'costs' | 'payment' | 'taxes') => {
